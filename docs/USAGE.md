...
执行者会按 [`deltas/intake-guide.md`](../deltas/intake-guide.md) 的引导协议推进：

1. **先问一句**："这份 PPT：是要快，还是要尽可能做好？"（默认：尽可能好）
2. **尽可能好** → 按内容量伸缩流程深度（≤12 页轻量稿 / >12 页或重要场景完整流程），
   **质检拉满**（四道自动检查 + 渲染目视 + 内容复核），展示 2–3 个视觉候选，
   演讲场景询问逐字稿/提词卡
3. **要快** → 轻量通道，按默认值出稿并**标注假设**
4. 全程遵守引导原则：**场景语言提问**（不用内部术语）、**随时可跳过**、
   **一次最多 3 问**、**能推断就不问**

> 完整协议、四条原则与两个示例对话见
> [`deltas/intake-guide.md`](../deltas/intake-guide.md)。

### 8.2 辅轨 · 精确模式（信息一次给全）

**何时用**：批量制作、团队协作需留档、已有明确素材路径或硬性约束。
**何时不用**：日常一句话需求——走 §8.1 即可。
...
**补充说明（可选）**：是否需要演讲者备注（逐字稿 or 提词卡）、是否有指定页型或配色、
交付形态（仅 .pptx / 需要缩略总览预览）。

> 用精确模式时，§8.1 的质量分支仍然生效——**模板只是把信息一次给全，不改变质量协议**。

### 8.3 执行者会做什么

定路线 → 产导演稿（若需）→ 选骨架填充 → 过质量门 → 导出 → 质检 →（可选）渲染。

**任何返修后必须重跑质检流程**（见 §2.2 的 C-028 提示）。
