...
## 2026-09-15 · T-GUIDE1 复核裁决（引导协议就绪）

### 复核结论

- `intake-guide.md` 200 行 7 节：二分入口 / 质量协议（伸缩+质检拉满+B3 默认）/ 快通道 / 4 原则 / **2 示例对话（指挥官逐行读过：对话自然、零术语、周报例 1 问直达、方案例 5 问含 3 个"你来定"）** ✅
- 执行者裁决采纳：**B3 内容复核不设轻量稿豁免**（C-023/C-024 类内容错误与页数无关）——同意，理由成立
- 术语分区处理正确（元说明区列禁词表 / 用户可见区全扫描 0 命中；首版 5 处漏网已改）
- 顺带修正三处失实描述（README/QUALITY-TIERS 的"24 骨架"→29、"不支持图片"已过时）——文档一致性维护 ✅
- 环境教训：PowerShell 5.1 对无 BOM UTF-8 按 GBK 解码 → 行数统计失真（W-2 又一实例）→ **行数/字符校验一律字节级**

### Backlog（新增）

- `check_terms.py`（术语审计常驻检查）——术语禁令不能靠人工记忆（首版漏 5 处为证），并入下批顺带
- 引导协议的"≤12 页阈值"待真实使用数据校准；视觉候选"可看图"机制待补

### 状态

**系统就绪：能力面 100% + 引导协议 + 四道闸 + 目视闭环。实测进入"一句话即可开始"状态。**
