...
## 2026-09-15 · T-IMG1 复核（含目视）裁决

### 复核结论

- 3 骨架入库（v3），端到端小样 6 页 / 3 页图片骨架 / 四道质检全过 / 读回 8 个 Picture shape 几何一致 ✅
- **图片内嵌而非链接**（包内 media 与源文件字节一致）——交付 .pptx 无需附带 images/ 目录 ✅
- **指挥官目视**：三种图片版式结构全部成立（标题带/左图右文/网格图注）；变形探针确认无拉伸、slice 裁切正确 ✅
- C-029（读回假阴性：顶层扫描漏 group 内 shape）——**"没找到"≠"不存在"**，已修（递归）；通用教训记录
- C-030（`meet` 缩 shape / `slice` 裁切）——契约行为已文档化
- **素材矩阵现实**：本环境 WSL 无外网 → 搜图不可用；生图 14 后端全需 key → **当前仅"用户提供 + 程序生成"两条素材路**（key 名已列，未申请）
- **图片内容对四道自动质检完全无感** → "图片页必须人工目视"列入永久纪律

### 项目能力面状态

**文字 + 图表（9 类）+ 图片（3 骨架）三要素齐备**；骨架库 29 个；测试集 6/6；视觉评分 4.5（真实素材引入后有望再升）。

### 下一步选项（待用户）

| 选项 | 说明 |
|---|---|
| A. 工程收尾批 | tables 族接入 + v2 骨架图表坐标自动化（§2.6 风险关闭）——完成后能力面 100% |
| B. 用户实测 | 用真实需求跑一轮（终极验收；反馈驱动最后收尾） |
| C. 公开化 | 仓库 Private → Public 复核（第三方资产表述已就绪） |
| D. 渲染器冗余 | 需用户授权安装 LibreOffice |

### 用户决策（后续）

- D（渲染器冗余）→ **不需要**（用户拒绝；WPS COM 单点风险保留记录）
- **A. 工程收尾批** → 发 `tasks/T-A1.md`（tables 接入 + v2 图表坐标自动化）

## 2026-09-15 · T-A1 复核裁决（能力面 100% 达成）

### 复核结论

- 表格：`fill_table.py` 把 6 模板原生标记能力拉齐（实测：生成版导出读回 2/2 原生 Table；base 原始仅 2/6 具备）✅
- v2 坐标自动化：`v2_chart_gen.py` 几何逐项比对 3/4 完全一致；line-chart 差异确认为**手写版自身缺陷**（C-033）✅
- **C-032 里程碑**：donut 手写骨架 50% vs 图例/脚注 75% 矛盾（文档同错）——**首次由自动化几何比对发现骨架缺陷**（此前靠人眼）✅
- **指挥官目视验收**：表格排版专业；v2 生成版与手写版视觉一致；donut 修复清晰可见 ✅
- C-034（GBK 控制台 emoji 崩溃）/ C-035（TSD 对 WSL 侧读 .py 同样加密 → **repo 文件操作一律走 Windows 侧**）记入环境教训

### 能力面 100% 宣言

**文字（29 骨架）+ 图表（9 类 + v2 自动化）+ 图片（3 骨架 + 内嵌链路）+ 表格（6 模板拉齐）四要素齐备；测试集 6/6；四道自动闸 + 目视闭环；36 commits。**

### 下一步（待用户）

| 选项 | 说明 |
|---|---|
| **A. 用户实测**（推荐） | 能力面已完成，终极验收 = 真实使用 |
| B. 公开化 | Private → Public 复核 |
| C. 后备增强 | 执行者建议：image-band/image-compare 扩充、feature/rating_matrix 原生标记补齐（均为锦上添花，可待实测反馈驱动） |

### 用户决策

**A. 用户实测** → **项目正式进入实测阶段**（能力面 100%，交付验收由用户真实使用完成）。
实测反馈 = 下一批任务卡的唯一输入源；后备增强（C）备案待用。

## 2026-09-15 · 实测反馈首批（引导协议）与 T-GUIDE1 发布

### 用户反馈（三条，直接驱动本卡）

1. 六输入模板**门槛高**（为 AI 省事而非为用户省事）→ 自然语言优先 + 追问协议
2. 需要**可跳过的引导流程**（帮助用户做出最好的产品）
3. **不选快速 = 尽可能高质量**（质量默认值向上）

### 指挥官设计裁决

- 用户界面收敛为**二分入口**（快 / 尽可能好）；术语不进对话
- "好"分支拆两层：**流程深度按内容量伸缩**（≤12 页轻量导演 / >12 页完整导演式）、
  **质检强度不打折**（四道闸 + 目视 + **B3 评审卡默认挂载**）
- 视觉候选必须展示（2-3 个看图选）；演讲场景询问逐字稿/提词卡

### 发布

`tasks/T-GUIDE1.md`：`deltas/intake-guide.md`（引导协议 + 2 个示例对话）+ USAGE §8 重构 + README 同步。
**这是实测阶段的第一张反馈卡——反馈→卡→执行→更顺的实测，机制本身开始运转。**
