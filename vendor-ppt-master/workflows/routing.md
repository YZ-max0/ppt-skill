...
## 8. Director Pre-spec Input Stage

**Hard rule — optional input stage, not a fifth top-level route**: The
Director workflow is an optional Generate PPTX *input* stage that runs before
planning (Step 0, before the Design Spec). It does not create a new artifact
lifecycle, does not appear in the §2 route matrix, and the four top-level
routes remain the only routes. This section only appends the trigger and the
integration contract for the Director stage; every existing row in §1–§7 is
unchanged.

**Trigger — Director activates before Generate PPTX planning when any of these
request shapes match**:

| Request shape | Director behavior |
|---|---|
| Topic/idea/incomplete material only | Run the Director workflow (audience → content → page structure → generation-ready mapping) |
| Important presentation (decision-makers / management / review board) and ≥10 pages | Run the Director workflow in controlled full form |
| Complete material, ≤8 pages, tight schedule | Use the quick lane (lightweight director draft straight into `quick-generate`) |
| Existing finished PPTX | Route to Enhance Native / Beautify as before; do not run the Director stage |
| User explicitly asks for simple/quick | Use the quick lane |

**Mount point and planning authority**: The Director stage is mounted as
Generate PPTX **Step 0 (pre-spec)**: its outputs are consumed as *input
material* by the Strategist phase. `design_spec.md` + `spec_lock.md` remain the
**only planning authorities**; the director draft is not a parallel planning
document and is discarded after Strategist consumes it. Any conflict between a
director draft and the confirmed Design Spec is resolved in favor of the Design
Spec / lock.

**Reference**: [`../../deltas/director.md`](../../deltas/director.md) (repo-level
D-1 Director workflow, resolved from `vendor-ppt-master/workflows/`).

**Boundary**: This stage adds no new route row, no new Confirm UI stage, and no
second user confirmation gate. The existing Generate Step-4 confirmation remains
the single human gate; the Director stage only prepares the material that feeds
that gate. §1–§7 of this file are not modified by this section.
