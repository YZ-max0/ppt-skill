...
**止损说明**：本方案（文本特征 subtier）经指挥官指定为**只给一次机会**——若在靶场或回归中
出现**第三类误报**（跨档误报、特征类误报之外的），则放弃特征方案，切换为「页面主标题通道」
（每页最大字号文本的跨页一致性比较，替代全量档内比较）。
**本次实测结果：靶场 6→0、全量回归 10/10、真阳性矩阵 3/3，未出现第三类误报 → 方案保留，未切换。**
