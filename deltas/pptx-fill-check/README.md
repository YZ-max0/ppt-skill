...
**档内二次切分：文本特征 subtier（T-FIX3 引入）**

同一字号档内仍可能混有**不同角色的文本**（如 18pt 说明句与 21pt 卡片标签）——这是 C-004/C-007
误报的根源。因此在档内再按文本形态三分：

| subtier | 判定 | 例 |
|---|---|---|
| `seq` | ASCII 编号令牌：纯数字或纯字母，长度 ≤ 4，无空白 | `01`、`1.`、`(3)`、`A`、`IV` |
| `short` | 非编号且视觉宽度 ≤ 5 vw（约 ≤5 个 CJK 字） | `风险`、`对策`、`验收标准`、`快速找到` |
| `long` | 其余（副标题、说明句） | `进度符合预期，两项风险已有对策` |

- 同页 spread 与跨页偏离**只在「同字号档 + 同 subtier」内比较**
- tier 标签形如 `text@24-28/short`，含 subtier 便于追溯
- 占位符路径**完全不变**（role 是 ground truth）
- 判定注意：CJK 标签（如 `风险`）的 `str.isalpha()` 为 True，故 `seq` 判定**必须限定 ASCII**

**效果（实测）**：
| deck | 修复前 P1 | 修复后 P1 |
|---|---|---|
| T-03（21 页） | 6（全误报） | **0** |
| T-01b（6 页） | 1（C-004 误报） | **0** |
| 真阳性矩阵（seq/short/long 各 1） | — | **3/3 正确报出** |

**R1 更新（2026-09-14）· subtier 第四类 `value`**

新增 `value` 类（数字 + 可选短单位：`40 万`、`1.8 年`、`99.4%`），从 `short` 分离。
分类器单元测试 20/20 通过。

### ⚠️ 硬止损已触发：free-design 路径切换为「页面主标题通道」

**止损条款**：特征方案累计给三次机会（FIX2 分档 → FIX3 subtier → R1 `value` 类）。
**任一次之后再出现 free-design 误报 → 无条件切换主标题通道，不再扩展特征。**

**触发经过**：`value` 类加入后，t03-v2 靶场**仍报 3 项误报**——
因为该 deck 的验收指标值是**文字**（`秒级返回`/`抽查全对`/`周活跃达标`），不是数字，
无法归入 `value`，仍与条目名同属 `short`。**→ 条款触发。**

**切换后的规则（当前生效）**：

| 路径 | 判据 |
|---|---|
| **占位符路径** | **完全不变**（role 是 ground truth，`center_title`/`title`/`subtitle`/`body`/`object`） |
| **自由设计路径** | **页面主标题通道**：每页只取**字号最大**的标题候选（同字号取较短文本），对这些页主标题做同页 spread 与跨页偏离比较 |

**为何这样切换**：主标题通道**永远不会把同页两个不同角色的文本放在一起比较**，
从机制上消除了 C-004/C-007/C-010 这一整类误报。

**已知代价（如实记录，不是缺陷）**：
- **同页非主标题的漂移不再被检测**。例：样张 `samepage_3`（同页 28pt×2 + 24pt×1）
  在切换后**不再报告**（此前报告 1 项）。
- 这是"用同页副标题检测能力换取误报消除"的有意取舍。
- 若将来需要同页多标题一致性，应作为**独立功能**设计，不回到特征方案。
...
|---|---|
| t03-v2 靶场 | **0**（修复前 3）✅ |
| t01/t01b/t03/t03v2 | 0 ✅ |
| sample_ok / p1 / p0 | 0 / 0 / 1 ✅ |
| smoke / samepage_2 | 0 / 0 ✅ |
| samepage_3 | 0（**已知代价**，原为 1） |
| truepos（跨页） | 1 ✅ |
| tp3_seq / tp3_short / tp3_long / **tp3_value** | 1 / 1 / 1 / **1** ✅ |
